$(function () {
	...
	defaultselectbox.after('<div class="select-label"></div>');

	// getting default select box selected value
	$('.select-label').text(defaultselectbox.children('option').eq(0).text());

	// appending options to custom un-ordered list tag
	var cusList = $('<ul/>', { 'class': 'options'} ).insertAfter($('.select-label'));
	...
	$(".options li").on('keypress click', function(e) {
		...
		$('.selected-item p span').text($('.select-label').text());
	});
